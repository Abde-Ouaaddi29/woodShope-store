  ...
  const handleSubmit = async (e) => {
    ...
    if (
      ...
      if (response) {
        ...
        setError("* issue info please try again !  ");
        setTimeout(() => {
          setError('');
        }, 6000);
        setIsRegistered(false);
        ...
        {isRegistered ? (
          <div className="bg-green-100 px-6 py-4 w-full mb-6 rounded flex items-center border border-green-200">
            ...
            <span className="text-green-600 tracking-wide mr-2">
              you have been regestered succesfully
            </span>{" "}
            <span>
        ...
        <div className="border border-yellow-500 shadow-xl lg:w-5/12 w-11/12 p-6 rounded-md flex flex-col justify-center items-center">
          <div className="w-10/12 flex justify-between items-center mb-3">
            <label className="font-bold " htmlFor="name">
              {/* <span className="text-red-800 tracking-wide"> {error} </span> */}
              name:
            </label>
            <input
              id="name"
              className={error ? 'w-8/12 p-1 outline-yellow-400 border-2 border-red-200' : 'w-8/12 p-1 outline-yellow-400 border-2 border-gray-200' }
              ref={name}
              type="name"
            />
          </div>
